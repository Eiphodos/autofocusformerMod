...
class SingleScaleMaskFinerTransformerDecoder(nn.Module):
    ...
    def forward(self, x, pos, mask_features, mf_pos, finest_input_shape, input_shapes):
        ...
        all_pos_emb = torch.cat(pos_emb, dim=0)
        all_src = torch.cat(src, dim=0)
        all_pos_scaled = torch.cat(poss_scaled, dim=1)

        outputs_class, pred_mask, attn_mask = self.forward_prediction_heads(output, mask_features, mf_pos_scaled, all_pos_scaled, masked_attn)  # b x q x nc, b x q x n, b*h x q x n
    ...
    def forward_prediction_heads(self, output, mask_features, mf_pos, target_pos, masked_attn):
        ...
        if masked_attn:
            ...
            attn_mask = (outputs_mask.sigmoid().unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) < 0.5).bool()  # b*h x q x n
