...
class MultiScaleMaskFinerTransformerDecoderCat(nn.Module):
    ...
    def forward(self, x, pos, mask_features, mf_pos, finest_input_shape, input_shapes):
        ...
        poss_cat = torch.cat(poss_scaled, dim=1)
        pos_emb_cat = torch.cat(pos_emb, dim=0)
        src_cat = torch.cat(src, dim=0)

        outputs_class, pred_mask, attn_mask = self.forward_prediction_heads(output, mask_features, mf_pos_scaled, poss_cat, masked_attn)  # b x q x nc, b x q x n, b*h x q x n
        ...
        for i in range(self.num_layers):
            ...
            output = self.transformer_cross_attention_layers[i](
                ...
                pos=pos_emb_cat, query_pos=query_embed
            )
