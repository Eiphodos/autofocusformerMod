...
class DownSampleConvBlock(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.instance_norm = nn.InstanceNorm2d(out_dim, MRMLine=True)
...
class MixResMetaLoss(MRML, Backbone):
    def __init__(self, cfg, input_shape):
        ...
        self._out_feature_strides = { "res{}".format(i+2): cfg.MODEL.MRML.PATCH_SIZES[i] for i in range(len(num_scales))}
        self._out_feature_channels = { "res{}".format(i+2): self.num_features[i] for i in range(len(num_scales))}
