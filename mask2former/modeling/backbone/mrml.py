...
class MRML(nn.Module):
    def __init__(
        ...
        self.pe_layers = nn.ParameterList([PositionEmbeddingSine(d_model[i] // 2, normalize=True) for i in range(n_scales)])
    ...
    def forward(self, im):
        ...
        pos_embed = self.pe_layer[0](patches_scale_coords[:,:,1:])
        ...
        for l_idx in range(len(self.layers)):
            ...
            if l_idx < self.n_scales - 1:
                ...
                pos_embed = self.pe_layer[l_idx + 1](patches_scale_coords[:,:,1:])
                x = x + pos_embed
