...
class MRMean(nn.Module):
    ...
    def divide_tokens_to_split_and_keep(self, tokens_at_curr_scale, patches_scale_coords_curr_scale, curr_scale):
        ...
        mean_act = tokens_at_curr_scale.mean(-1)
        tkv, tki = torch.topk(mean_act, k=k_split, dim=1, sorted=False)
        bkv, bki = torch.topk(mean_act, k=k_keep, dim=1, sorted=False, largest=False)
    ...
    def split_input(self, tokens, patches_scale_coords, curr_scale, patch_size, im):
        ...
        tokens_to_split, coords_to_split, tokens_to_keep, coords_to_keep = self.divide_tokens_to_split_and_keep(
...
class MixResMeanAct(MRMean, Backbone):
